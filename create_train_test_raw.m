...
users_ratio = 20/86; %percentage of users for train set
test_users = 5;
bsamples = 60; %number of baselines per user
trial_ratio = 1; %percentage of trials for use in train set
...
samples = T*fs;
users_train = [42,32,7,53,54,2,49,56];%sort(randperm(N,floor(users_ratio*N)));
users_test = 1:N;
...
%         b = repmat(b(channels,:),1,samples);
        ...
        for code=1:2
            if (code == 1)
                set = left_raw{user,1};
                train_trials = 1:size(set,1);%sort(randperm(size(set,1),floor(trial_ratio*size(set,1))));
            elseif (code == 2)
            ...
            for trial=1:ntrials
                ...
                start = round(sz/2-samples/2);%ceil(rand*(sz-samples-1));
...
% perform the preprocessing on the training data
 %preprocessing options 'spatialfilter','none'
...
% [Data,train,mul,coeffl] = train_preprocessing_ersp(train, opt, pca_th, ppca);
...
% Export the files
save('dataset/processedDataset_raw.mat','train','train_users','labelstr','ch_pos','frequency_band','fs','channels');
